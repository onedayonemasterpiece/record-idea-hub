...
class SyncWorker(appContext: Context, parameters: WorkerParameters) : CoroutineWorker(appContext, parameters) {
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        if (serverUrl.isNullOrBlank() || deviceToken.isNullOrBlank()) return@withContext Result.success()
        ...
        try {
            ...
            for (session in store.sessionsNeedingSync()) {
                ...
                if (session.protocolVersion >= 2) syncV2(store, ApiClientV2(serverUrl, deviceToken), session)
                else syncLegacyV1(store, ApiClientV1(serverUrl, deviceToken), session)
            }
            Result.success()
            ...
            currentSessionId?.let {
                store.setRemoteState(it, RemoteState.RETRYABLE_ERROR,
    ...
    private fun syncV2(store: SessionStore, api: ApiClientV2, original: SessionSnapshot) {
        ...
        if (initialized.githubVerified) { store.deleteVerifiedAudio(original.sessionId); return }
        ...
        check(chunks.size == session.chunkCount) { "local chunk count changed before v2 completion" }
        if (chunks.any { !it.uploaded }) { SyncScheduler.enqueue(applicationContext, 15L); return }
        val progress = if (!session.completeSent) {
            api.completeSession(session, chunks).also { store.markCompleteSent(session.sessionId) }
        } else api.status(session.sessionId)
        store.updateRemoteProgress(session.sessionId, progress)
        when {
            progress.githubVerified -> store.deleteVerifiedAudio(session.sessionId)
            progress.reconciliationRequired -> store.setRemoteState(
                session.sessionId, RemoteState.RECONCILIATION_REQUIRED,
                "Сервер сверяет уже отправленный запрос; аудио сохранено")
            progress.retryable && progress.retryAfterSeconds != null -> {
                store.setQuotaWait(session.sessionId, progress.retryAfterSeconds,
                    progress.lastError ?: progress.errorCode ?: "Ожидание сервера")
                SyncScheduler.enqueue(applicationContext, progress.retryAfterSeconds.toLong())
            }
            else -> SyncScheduler.enqueue(applicationContext, store.nextPollDelaySeconds(session.sessionId))
        }
    }
    ...
    private fun syncLegacyV1(store: SessionStore, api: ApiClientV1, original: SessionSnapshot) {
        ...
        if (session.captureState == CaptureState.FINISHED) {
            ...
            if (reconciled.githubVerified) { store.deleteVerifiedAudio(session.sessionId); return }
        }
        for (chunk in store.pendingChunks(session)) {
            store.setRemoteState(session.sessionId, RemoteState.PROCESSING)
            store.saveChunkTranscript(session.sessionId, chunk.chunkIndex, api.uploadChunk(chunk))
        }
        session = store.session(session.sessionId) ?: return
        if (session.captureState != CaptureState.FINISHED || session.chunkCount <= 0) return
        val chunks = store.chunks(session.sessionId)
        if (chunks.any { !it.transcribed }) { SyncScheduler.enqueue(applicationContext, 15L); return }
        store.setRemoteState(session.sessionId, RemoteState.PUBLISHING)
        val progress = api.completeSession(session, chunks)
        store.updateRemoteProgress(session.sessionId, progress)
        if (progress.githubVerified) store.deleteVerifiedAudio(session.sessionId)
        else SyncScheduler.enqueue(applicationContext, 15L)
    }
    ...
    private fun handleApiFailure(store: SessionStore, sessionId: String, exc: ApiException) {
        ...
        if (exc.code == "provider_429" || exc.code.startsWith("quota_exhausted_"))
            store.setQuotaWait(sessionId, delay, exc.message ?: "Ожидание доступного лимита Gemini")
        else store.setRemoteState(sessionId, RemoteState.RETRYABLE_ERROR, exc.message)
